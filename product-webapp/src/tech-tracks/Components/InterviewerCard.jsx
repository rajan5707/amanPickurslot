...
const InterviewerCard = (props) => {
  ...
  const postData = (data, description) => {
    axios
      .post("http://13.235.16.107:8080/tagservice/api/v1/bookedSlot", {
        // id: data.id,
      ...
      .then((response) => {
        if(response.status===201){
          axios.put(`http://13.235.16.107:8080/interviewerservice/api/v1/updateSlot`,{
            slotId: data.slotId,
  ...
  const ShowSlotsModal = (props) => {
    ...
    const fetchData = () => {
      ...
      axios
        .get(
          `http://13.235.16.107:8080/interviewerservice/api/v1/availableSlot/${modalShow.email}`
        )
        .then((res) => {
          return res.data;
        })
